...
public class TournamentService : ITournamentService
{
    ...
    public async Task UpdateContractAmountAsync(int id, int sponsorId, decimal contractAmount)
    {
        var tournament = await _tournamentRepository.GetByIdAsync(id);
        var tournamentSponsor = await _tournamentSponsorRepository.GetByTournamentAndSponsorAsync(id, sponsorId);
        if (tournament == null)
            throw new KeyNotFoundException($"No se encontró el torneo con ID {id}");

        // Solo se puede actualizar el patrocinador en torneos Pending
        if (tournament.Status != TournamentStatus.Pending)
        {
            throw new InvalidOperationException(
            "Solo se puede actualizar patrocinadores en torneos con estado Pending");
        }

        // Validar que esté inscrito el patrocinador
        var existing = await _tournamentSponsorRepository
        .ExistsSponsorOnTournamentAsync(id, sponsorId);
        if (!existing)
        {
            throw new InvalidOperationException(
        ...
        tournamentSponsor!.ContractAmount = contractAmount;

        _logger.LogInformation(
        "Updating sponsor {sponsorId} amount contract to {contractAmount}",
        id, contractAmount);
        await _tournamentSponsorRepository.UpdateAsync(tournamentSponsor);
    }

    public async Task UpdateSponsorCategoryAsync(int id, int sponsorId, SponsorCategory category)
    {
        var tournament = await _tournamentRepository.GetByIdAsync(id);
        var tournamentSponsor = await _tournamentSponsorRepository.GetByTournamentAndSponsorAsync(id, sponsorId);
        if (tournament == null)
        ...
        tournamentSponsor!.Category = category;

        _logger.LogInformation(
        "Updating sponsor {sponsorId} category to {category}",
        id, category);
        await _tournamentSponsorRepository.UpdateAsync(tournamentSponsor);
    }
    ...
    public async Task RegisterSponsorAsync(int tournamentId, int sponsorId, decimal contractAmount, SponsorCategory category)
    {
        // Validar que el torneo existe
        var tournament = await _tournamentRepository.GetByIdAsync(tournamentId);
        if (tournament == null)
        ...
        // Validar que no esté ya inscrito
        var existing = await _tournamentSponsorRepository
        .ExistsSponsorOnTournamentAsync(tournamentId, sponsorId);
        if (existing)
        {
            throw new InvalidOperationException(
