...
public class TournamentController : ControllerBase
{
    ...
    public async Task<ActionResult> UpdateCategory(int id, int sponsorId ,UpdateCategoryDTO dto)
    {
        try
        {
            await _tournamentService.UpdateSponsorCategoryAsync(id, sponsorId,dto.Category);
            return NoContent();
        }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
    ...
    [HttpPatch("{id}/contractAmount")]
    public async Task<ActionResult> UpdateContractAmount(int id, int sponsorId ,UpdateContractAmountDTO dto)
    {
        try
        {
            await _tournamentService.UpdateContractAmountAsync(id, sponsorId,dto.ContractAmount);
            return NoContent();
        }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
    ...
    public async Task<ActionResult> RegisterSponsor(int id,RegisterSponsorDTO dto)
    {
        try
        {
            await _tournamentService.RegisterSponsorAsync(id, dto.SponsorId, dto.ContractAmount, dto.SponsorCategory);
            return Ok(new { message = "Patrocinador inscrito exitosamente" });
        }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
    }
